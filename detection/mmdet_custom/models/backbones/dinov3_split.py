...
class DINOv3SplitFusion(BaseModule):
    """ViT-Split style frozen-feature + split-head fusion for DINOv3 detection."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.norm1 = nn.SyncBatchNorm(self.channels)
    ...
    def forward(self, x):
        ...
        x = self.fusion_net(torch.cat([frozen_features, tuned_features], dim=1))
        return [
            self.norm1(self.up1(x)),
            self.norm2(self.up2(x)),
            self.norm3(self.up3(x)),
            self.norm4(self.up4(x)),
        ]
