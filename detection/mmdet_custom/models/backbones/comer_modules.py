...
class CTI_toC(nn.Module):
    ...
    def forward(self, query, reference_points, feat, spatial_shapes, level_start_index, H, W):
        ...
        def _inner_forward(query, feat, H, W):
            ...
            query = torch.cat([x1, x2 + feat, x3], dim=1)
        ...
        if self.with_cp and query.requires_grad:
            query = cp.checkpoint(_inner_forward, query, feat, H, W, use_reentrant=False)
        else:
...
class CTI_toV(nn.Module):
    ...
    def forward(self, query, reference_points, feat, spatial_shapes, level_start_index, H, W):
        def _inner_forward(query, feat, H, W):
            ...
            c_select1 = c1[:, :H * W * 4, :]
            c_select2 = c1[:, H * W * 4:H * W * 4 + H * W, :]
            c_select3 = c1[:, H * W * 4 + H * W:, :]
            c_select1 = F.interpolate(
                c_select1.permute(0, 2, 1).reshape(B, C, H * 2, W * 2),
                scale_factor=0.5, mode='bilinear', align_corners=False).flatten(2).permute(0, 2, 1)
            c_select3 = F.interpolate(
                c_select3.permute(0, 2, 1).reshape(B, C, H // 2, W // 2),
                scale_factor=2, mode='bilinear', align_corners=False).flatten(2).permute(0, 2, 1)
...
class CTIBlock(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 ...
                 use_CTI_toV=True, use_CTI_toC=True, dim_ratio=0.5,
                 cnn_feature_interaction=False, extra_num=4):
        super().__init__()
        if use_CTI_toV:
