...
@PIPELINES.register_module()
class ToMaskFromInstanceMap(object):
    """Use instance ids to generate ``gt_masks`` and ``gt_labels``."""
    ...
    def __call__(self, results):
        gt_semantic_seg = results['gt_semantic_seg']
        instance_map = results.get('gt_instance_map')
        if instance_map is None:
            raise KeyError('gt_instance_map is required for ToMaskFromInstanceMap')
        ...
        for inst_id in instance_ids:
            ...
            pixel_labels = gt_semantic_seg[mask]
            pixel_labels = pixel_labels[pixel_labels != self.ignore_index]
            if pixel_labels.size == 0:
                continue
            unique_labels = np.unique(pixel_labels)
            if unique_labels.size != 1:
                raise ValueError(
                    f'mixed semantic labels found for instance id {int(inst_id)}: '
        ...
        if masks:
            gt_masks = np.stack(masks, axis=0).astype(np.int64)
            gt_labels = np.asarray(labels, dtype=np.int64)
        else:
            mask_shape = gt_semantic_seg.shape
            gt_masks = np.empty((0,) + mask_shape, dtype=np.int64)
            gt_labels = np.empty((0,), dtype=np.int64)
